...
class MemoryVLA(nn.Module):
    ...
    def predict_action(
        ...
        **kwargs: str
        ...
        # [FIX · 2026-07-25] Piper 训练标签的第 7 维是连续绝对开度（约 79.6% 为中间值，
        # 且 action_normalization_mask 对该维为 False），硬 0/1 二值化会把 0.49/0.51 打成
        # 0mm/70mm 满行程跳变。设 MEMVLA_BINARIZE_GRIPPER=0 时保留连续开度（robokit
        # 部署端默认设置）；缺省 =1 保持上游原行为，兼容其他评测脚本。
        import os as _os
        if _os.environ.get("MEMVLA_BINARIZE_GRIPPER", "1") != "0":
